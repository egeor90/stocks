...
for(i in 1:length(stocks_)){
  ...
  assign(tolower(stocks_[i]),read.csv(paste0(".../",stocks_[i],"_1min.csv"), header = T, stringsAsFactors = F))
...
nasdaq <- read.csv(paste0(".../NASDAQ_1min.csv"), header = T, stringsAsFactors = F)
